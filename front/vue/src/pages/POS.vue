		...
		.layer
			...
			.partner-check(v-show="status.showPartner")
				.inner
					.title
						|¿Es socio?
					div.btn.btn-danger.cancel-btn(v-focus, data-down="es_socio", data-up="AUTOFOCUS", id="partner_no", data-right="partner_yes", @keyup.enter="_partnerNo", @click="partnerNo", @keyup.esc="exit", tabindex="200", ref="partner_no", v-if="stage == 'partnering'")
						|No
			...
			.biometric-scan(v-show="status.showBiometric")
				.inner
					...
					div.btn.btn-primary.btn-block.mt-3.scanner-button(v-focus, data-down="AUTOFOCUS", data-up="AUTOFOCUS", id="scan_fingerprint", @keyup.enter="_scanFingerprint", @click="scanFingerprint", @keyup.esc="exit", tabindex="202", ref="scan_fingerprint", v-if="stage == 'biometric_scanning'")
						span
							|{{ scanButtonText }}
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.axios.get( url, config )
			.then( result => {
				let fixedProducts = result.data.map(p => {
					return {
						_id: p._id,
						name: p.name,
						sku: p.sku,
						...
						id_scanner: p.id_scanner,
						pesable: p.pesable,
					}
				})
				localStorage.normalProducts = JSON.stringify( fixedProducts )
			})
		...
		methods: {
			...
			confirmOrder: function() {
				...
				body.order_total = parseFloat( Number( body.order_total ) ).toFixed(2)
				...
				.then( result => {
					...
					if( createdOrder.mensaje && createdOrder.mensaje.match(/suficiente/g)){
						navigationService.getFocusElementById("p_0").focus()
						return this.$swal("Stock en sistema insuficiente", createdOrder.mensaje, "error")
					}else{
						return this.$swal({
							title: "Orden " + (result.data.identifier) + " confirmada",
							text: "",
			...
			printTicket: function(order){
				let printURL = this.servlet + "Impresora"
				...
				console.log(JSON.stringify(trimmedOrder));
			...
			_pay: function(index) {
				console.log("_pay")
				//if(index == this.order.products.length +1)
				this.$refs.pay.click()
			},
			_partnerNo: function() {
				console.log("_pn")
